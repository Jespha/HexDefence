...
{
	[SerializeField]
	private PooledObject _hpBar;

	[SerializeField]
	private EnemyManager _enemyManager;

	[SerializeField]
	private List<PooledObject> _hpBars = new List<PooledObject>();

	[SerializeField]
	private List<Image> _hpBarImages = new List<Image>();

	[SerializeField]
	private List<TextMeshProUGUI> _hpBarText = new List<TextMeshProUGUI>();

	[SerializeField]
	private List<GameObject> _activeEnemyObjects = new List<GameObject>();
	...
	private IEnumerator WaitForEnemyManager()
	{
		yield return new WaitUntil(
			() => GameManager.Instance != null && GameManager.Instance.EnemyManager != null
		);
	...
	private void AddHpBar(GameObject enemyObject)
	{
		...
		hpBarText.text =
			_enemyManager.ProvideEnemyFloatData(enemyObject, "Health").ToString()
			+ "/"
			+ _enemyManager.ProvideEnemyFloatData(enemyObject, "MaxHealth").ToString();
		_hpBarText.Add(hpBarText);

		// Set the health bar's position to follow the enemy
		hpBar.transform.position = Camera.main.WorldToScreenPoint(
			enemyObject.transform.position + new Vector3(0, 2, 0)
		);
	}
	...
	private void UpdateHpBar(GameObject enemyObject, int index)
	{
		if (_hpBars[index])
		{
			...
			{
				_hpBars[index].transform.position = Camera.main.WorldToScreenPoint(
					enemyObject.transform.position + new Vector3(0, 2, 0)
				);
