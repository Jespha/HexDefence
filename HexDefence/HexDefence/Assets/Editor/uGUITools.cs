...
public class uGUITools : MonoBehaviour {
	[MenuItem("uGUI/Anchors to Corners %#r")]
	static void AnchorsToCorners(){
		foreach(Transform transform in Selection.transforms){
			...
			Undo.RecordObject(transform, "uGuiTools: Anchors To Corners");
	...
	[MenuItem("uGUI/Corners to Anchors %#t")]
	static void CornersToAnchors(){
		foreach(Transform transform in Selection.transforms){
			...
			Undo.RecordObject(transform, "uGuiTools: Corners to Anchors");
			t.offsetMin = t.offsetMax = new Vector2(0, 0);
		}
	}
	...
	static void MirrorHorizontally(bool mirrorAnchors){
		foreach(Transform transform in Selection.transforms){
			...
			Undo.RecordObject(transform, "uGuiTools: Mirror Horizontally");
			if (mirrorAnchors){
	...
	static void MirrorVertically(bool mirrorAnchors){
		foreach(Transform transform in Selection.transforms){
			...
			Undo.RecordObject(transform, "uGuiTools: Mirror Vertically");
