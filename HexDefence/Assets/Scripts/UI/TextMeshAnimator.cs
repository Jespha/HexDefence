...
namespace BBX.Dialogue.GUI
{
	public class TextMeshAnimator : MonoBehaviour
	{
		...
		private readonly Dictionary<int, Vector3[]> _defaultShakeVertices =
		...
		private void Animate(TMP_TextInfo textInfo, TMP_LinkInfo link)
		{
			...
			for (var i = start; i < end; i++)
			{
				...
				textInfo.meshInfo[materialIndex].mesh.vertices = textInfo
		...
		private void Wave(IList<Vector3> destinationVertices, int vertexIndex)
		{
			for (byte corner = 0; corner < 4; corner++)
			{
				var offset = WaveVector(
					WaveAmount,
					_frameCount * WaveSpeed
						+ destinationVertices[vertexIndex + corner].x / WaveSeparation
				);
		...
		private void Shake(IList<Vector3> destinationVertices, TMP_LinkInfo link, int vertexIndex)
		{
			...
			for (byte corner = 0; corner < 4; corner++)
			{
				destinationVertices[vertexIndex + corner] =
					_defaultShakeVertices[vertexIndex][vertexIndex + corner] + offset;
			}
		}
