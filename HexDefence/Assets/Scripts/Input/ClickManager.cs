...
public class ClickManager : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			_hexGridManager = FindFirstObjectByType<HexGridManager>();
	...
	{
		// Skip if no valid temp building is set
		if (GameManager.Instance.TempBuilding == null)
			return;
		...
		bool isValidBuildCell =
			hexCell.RoadEntryPoint == null
			&& hexCell.HexBuilding.HexBuildingType == HexBuildingType.None
			&& !hexCell.IsTemp;

		if (!isValidBuildCell)
			return;

		// Handle temp building placement
		if (_currentBuildHexCell != null)
		{
			if (hexCell == _currentBuildHexCell)
			{
				// Recreate the temp building to ensure it updates
				try
				{
				...
				{
					Debug.LogError($"Error updating temp building: {e.Message}");
				}
				return;
			}

			// Different cell - remove previous temp building
			_currentBuildHexCell.RevertTempBuilding();
		}

		// Set new temp building on the cell
	...
	private void OnLandClick(RaycastHit _hit, HexCell hexCell)
	{
		...
		{
			if (
				Currency.Instance.GoldCurrency >= GameManager.Instance.TempBuilding.Cost
				&& hexCell.HexBuilding.HexBuildingType == HexBuildingType.None
				&& hexCell.IsTemp == false
			)
			{
				_currentBuildHexCell.BuildHexBuilding(GameManager.Instance.TempBuilding);
				Currency.Instance.UpdateCurrency(
					-GameManager.Instance.TempBuilding.Cost,
