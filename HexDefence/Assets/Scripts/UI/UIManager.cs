...
public class UIManager : MonoBehaviour
{
	...
	[SerializeField]
	private EventReference buildSound;
	...
	private void Start()
	{
		...
		{
			buildingButtons = FindFirstObjectByType<BuildingButtons>();
			if (buildingButtons == null)
			{
	...
	private void OnHexSelectedUI(HexCell hexCell, RaycastHit hit)
	{
		...
		if (hexCell.HexBuilding == null || hexCell.HexTerrain == null)
		{
			Debug.LogWarning($"Hex cell at {hexCell.Position} is missing required components");
			selectedHexCell.UnSetSelectedHexCell();
			return;
		}
