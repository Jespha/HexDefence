...
public class PooledObjectManager : MonoBehaviour
{
	...
	void Start()
	{
		...
		{
			_clickManager = FindFirstObjectByType<ClickManager>();
	...
	public void AddToPool(PooledObject _pooledObject, int _amount)
	{
		...
		foreach (
			HexBuilding hexBuilding in Resources.LoadAll<HexBuilding>(
				"ScriptableObjects/HexBuilding"
			)
		)
		{
	...
	private void SetTempMaterials(PooledObject obj)
	{
		...
		{
			meshRenderer.material = Resources.Load<Material>(
				"ScriptableObjects/HexBuilding/TowerTempMat"
			);
		}
