...
public class GameManager : MonoBehaviour
{
	...
	public static UnlockedUpgrades UnlockedUpgradesInstance;
	...
	void Awake()
	{
		...
		UnlockedUpgradesInstance = Resources.Load<UnlockedUpgrades>(
			"ScriptableObjects/Upgrade/_UnlockedUpgrades"
		);
	}
	...
	private IEnumerator StartProgam()
	{
		...
		HexBuilding _arrowTower = Resources.Load<HexBuilding>(
			"ScriptableObjects/HexBuilding/ArrowTower"
		);
		HexBuilding _mortarTower = Resources.Load<HexBuilding>(
			"ScriptableObjects/HexBuilding/MortarTower"
		);
		TowerManager.UnlockTower(_arrowTower);
		TowerManager.UnlockTower(_mortarTower);
	}
	...
	private IEnumerator StartGameCoroutine()
	{
		...
		UnlockedUpgradesInstance.InitializeCurrentUnlockedUpgrades();
		...
		Currency.Instance.UpdateCurrency(
			...
			AnimationCoroutine
				.RectTransformToScreenSpace(
					UIManager.levelDisplay.LevelCompleteCurrencyAnimationParent[0].LocalRect
				)
				.position
		);
	...
	public IEnumerator LevelComplete()
	{
		...
		Currency.Instance.UpdateCurrency(
			CurrentLevel.hexCurrency,
			CurrencyType.HexCurrency,
			AnimationCoroutine
				.RectTransformToScreenSpace(
					UIManager.levelDisplay.LevelCompleteCurrencyAnimationParent[1].LocalRect
				)
				.position
		);
		Currency.Instance.UpdateCurrency(
			CurrentLevel.goldCurrency,
			CurrencyType.GoldCurrency,
			AnimationCoroutine
				.RectTransformToScreenSpace(
					UIManager.levelDisplay.LevelCompleteCurrencyAnimationParent[2].LocalRect
				)
				.position
		);
		;
	...
	public void GameOver(bool _restart = false)
	{
		EnemyManager.ClearEnemies();
		RoadManager.Instance.ClearRoads();
		HexGridManager.Instance.ClearHexGrid();
		if (_restart)
			StartGame();
	}
		...
		if (
			Vector3.Distance(
				new Vector3(
					...
					hexCell.transform.position.y,
					hexCell.transform.position.z
				),
				hexCell.transform.position
			) > 16f
		)
			FollowTarget.transform.position = new Vector3(
				hexCell.transform.position.x,
				FollowTarget.transform.position.y,
				-FollowTarget.transform.position.y + hexCell.transform.position.z
			);
	}
